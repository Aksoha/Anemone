...
public class AlgorithmParameterValidatorBase<T> : AbstractValidator<T> where T : MatchingParametersBase
{
    public AlgorithmParameterValidatorBase()
    {
        ...
        RuleFor(x => x).Must(x =>
            BeSinglePointOrRangeInAscendingOrder(x.FrequencyMin, x.FrequencyMax, x.FrequencyStep));

        RuleFor(x => x.TemperatureStep).GreaterThanOrEqualTo(0);
        RuleFor(x => x).Must(x =>
            BeSinglePointOrRangeInAscendingOrder(x.TemperatureMin, x.TemperatureMax, x.TemperatureStep));

        RuleFor(x => x.TurnRatioMin).GreaterThan(0);
        RuleFor(x => x.TurnRatioStep).GreaterThanOrEqualTo(0);
        RuleFor(x => x).Must(x =>
            BeSinglePointOrRangeInAscendingOrder(x.TurnRatioMin, x.TurnRatioMax, x.TurnRatioStep));
