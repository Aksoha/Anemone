...
LlcTopology::LlcTopology(std::shared_ptr<HeatingSystem> heatingSystem, const double inductance,
...
double LlcTopology::ParallelReactance(const double frequency, const double temperature) const
{
	...
	const auto xp = (w * inductance * (1 - w * w * inductance * Capacitance) - w * Capacitance * resistance *
			resistance) /
...
double LlcTopology::Impedance(const double frequency, const double temperature) const
{
	...
	const auto z = std::complex(resistance, reactance);
